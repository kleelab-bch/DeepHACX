setwd("//research.wpi.edu/leelab/Chauncey/Projects/paper_unequal_length/Experiments_truncated_data_analysis/01-09-2020-Across_DMSO_DeepHACKS")
...
filename <- "dist_truncated.mat"
data_mat <-readMat(filename)
dist <- data_mat$Dist.DeepFeature.truncated

dat_dist <- as.matrix(dist)

for (K_par in seq(1150, 1500, 50)){
  ...
  writeMat(paste("DMSO_total_", toString(K_par), "_truncated_community.mat", sep = ""), cluster_label = as.vector(cluster))
}
