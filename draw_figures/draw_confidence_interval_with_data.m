function [mean_pro, lower_pro, upper_pro] = draw_confidence_interval_with_data(data, name, event)
%draw data_.
%
%
...
confplot(1:1:length(mean_pro), mean_pro, lower_pro, upper_pro,'Color',[1 0 0],'LineWidth',1);


%plot all the data with transparent.
[row, col] = size(data_);
for i = 1 : row
    z = zeros(1, col);
    S = surface([1:col;1:col], [data_(i,:);data_(i,:)], [z;z], 'facecol', 'no', 'edgecol', 'interp', 'linew', 2, 'edgealpha', .02, 'edgecolor', 'c');
    hold on
end
